...
GBCore.Commands.Add("code", "Collect reward from code", {{name="Code", help="Enter Code"}}, true, function(src, args)
    MySQL.Async.execute('SELECT * FROM codes WHERE cuxcode = @playerCode', {['@playerCode'] = args[1]}, function(result)
        local xPlayer = GBCore.Functions.GetPlayer(src)
        ...
        if status == 0 then 
            if type == 'money' then
                ...
                --TriggerClientEvent('GBCore:Notify', src, "Yay! You redeemed the code!")
                TriggerClientEvent('esx:showHelpNotification', src, "Yay! You redeemed the code!")
		SendRedeemLog(code, type, amount, steamid)
                MySQL.Async.execute(false, "UPDATE `codes` SET cuxstatus = 1, cuxusedby = '"..steamid.."' WHERE `cuxcode` = '"..code.."'")
            else
		...
		SendRedeemLog(code, type, amount, steamid)
            ...
            TriggerClientEvent('GBCore:Notify', src, "This code is already used!")
        end
...
GBCore.Commands.Add("createcode", "Create a code with a reward", {{name="type", help="money or item name"}, {name="amount", help="Amount"}}, true, function(src, args)
	    ...
	    SendMadeLog(output, type, amount, steamid)
		    ...
		    MySQL.Async.execute(true, "INSERT INTO `codes` (`cuxcode`, `cuxtype`, `cuxamount`, `cuxstatus`, `cuxmadeby`) VALUES ('"..output.."', '"..type.."', '"..amount.."', 0, '"..steamid.."')")
